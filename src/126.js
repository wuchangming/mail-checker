...
function get126Url(email) {
	var url = 'http://entry.mail.126.com/cgi/ntesdoor?' +
		'hid=10010102&lightweight=1&language=-1&style=-1&username=';
	url += email;
	return url;
}

function is126Url(url) {
	// This is the Gmail we're looking for if:
	// - starts with the correct gmail url
	// - doesn't contain any other path chars
	var email = "mail.126.com/js3/main.jsp";
...
function get126InboxCount(onSuccess, onError) {
	...
	try {
		...
		xhr.onreadystatechange = function() {
			...
			if(xhr.status === 200) {
				...
				var url = get126Url(localStorage.email);
				console.debug("url: " + url);
				...
				xhr2.onreadystatechange = function() {
					...
					if(xhr2.status === 200) {
						chrome.cookies.getAll({ url: "http://mail.126.com/", name: "Coremail"}, function (cookies) {
